...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^crimson_") ; then
       BEHELIT_PRODUCT=$(echo -n $1 | sed -e 's/^crimson_//g')
    else
       BEHELIT_PRODUCT=
    fi
      export BEHELIT_PRODUCT
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${BEHELIT_DEVICES_ONLY}" != "z" ]; then
...
{
    target=$1
    BEHELIT_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/behelit/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the BEHELIT model name
            lunch crimson_$target-userdebug
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        MODVERSION=`sed -n -e'/ro\.behelit\.version/s/^.*=//p' $OUT/system/build.prop`
...
function taco() {
    ...
    do
        ...
        if [ $? -eq 0 ]; then
            croot
            ./vendor/behelit/bot/build_device.sh crimson_$sauce-userdebug $sauce
        else
            echo "No such item in brunch menu. Try 'breakfast'"
        fi
    done
